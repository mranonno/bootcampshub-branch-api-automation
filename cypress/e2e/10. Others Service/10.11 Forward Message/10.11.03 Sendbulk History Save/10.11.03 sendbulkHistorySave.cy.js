describe("Save send bulk history with status code 200", () => {
  let accessToken;
  let branchId;
  before(() => {
  ...
  it("Checking if should be able to save send bulk history", () => {
    cy.request({
      method: "POST",
      url: "/chat/sendbulk/history/save",
      headers: {
      ...
      if (response.status === 200) {
        ...
        cy.writeFile("cypress/fixtures/bulkId.json", {
          bulkId: body.history._id,
        });
