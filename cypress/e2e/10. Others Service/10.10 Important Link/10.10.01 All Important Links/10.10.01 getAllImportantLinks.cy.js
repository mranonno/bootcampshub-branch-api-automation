describe("Get all importance link with status code 200", () => {
  let accessToken;
  let branchId;
  before(() => {
  ...
  it("Checking if should be able to get all importance link", () => {
    cy.request({
      method: "GET",
      url: "/link/getall?page=1&limit=10&sort=newest&program=&session=&query=",
      headers: {
