describe("Get all transaction with status code 200", () => {
  let accessToken;
  let branchId;
  before(() => {
  ...
  it("Checking if should be able to get all transaction", () => {
    cy.request({
      method: "GET",
      url: "/transaction/getall?page=1&limit=10&program=&session=&status=&method=&query=",
      headers: {
